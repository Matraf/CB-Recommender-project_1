...
class DataPreprocessingToolkit(object):
    ...
    def filter_out_company_clients(df):
        ...
        :rtype: pd.DataFrame
        """
        return df.loc[(df["is_company"] == 0)].reset_index()
    ...
    def filter_out_low_prices(df):
        ...
        :rtype: pd.DataFrame
        """
        return df.loc[(df["accommodation_price"] > 50)].reset_index()
    ...
    def fix_date_to(df):
        ...
        """
        df_copy = df.copy()
        df_copy["date_to"] = df_copy["date_to"] + pd.DateOffset(days=1)
        return df_copy
    ...
    def add_length_of_stay(df):
        ...
        df_copy["length_of_stay"] = (df_copy["date_to"] - df_copy["date_from"]).dt.days
        return df_copy
    ...
    def add_weekend_stay(df):
        ...
        df['weekend_stay'] = (((s >= 4) & (s != 6)) | (e >= 5) | ((e < s) & (s != 6)) | (dt >= 6)).apply(str)
        return df
    ...
    def aggregate_group_reservations(self, df):
        ...
        dataset = [group_reservations.loc[:, ["group_id"] + self.sum_columns].groupby("group_id").sum(),
                   group_reservations.loc[:, ["group_id"] + self.mean_columns].groupby("group_id").mean(),
                   group_reservations.loc[:, ["group_id"] + self.mode_columns].groupby("group_id").agg(
                       lambda x: x.value_counts().index[0]),
                   group_reservations.loc[:, ["group_id"] + self.first_columns].groupby("group_id").first()]

        group_reservations = dataset[0]
        for i in range(1, len(dataset)):
            group_reservations = group_reservations.merge(dataset[i], on="group_id")
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        :rtype: pd.DataFrame
        """
        night_prices = df.loc[df['accommodation_price'] > 1] \
